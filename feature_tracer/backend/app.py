...
def download_template():
    """
    API endpoint to download an empty Excel template.
    """
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        pd.DataFrame(columns=['id', 'name', 'total_features', 'completed', 'in_progress', 'not_started', 'icon']).to_excel(writer, sheet_name='domains', index=False)
        pd.DataFrame(columns=['id', 'name', 'description', 'status', 'domain_id']).to_excel(writer, sheet_name='features', index=False)
    output.seek(0)
    return send_file(output, download_name='features_template.xlsx', as_attachment=True)
